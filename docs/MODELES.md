...
- **Notes importantes / Important notes**:
  ...
  - Les variantes **E2B/E4B** ("Efficient") tiennent confortablement dans WebGPU. Le **26B A4B** a un port ONNX communautaire mais **a été retiré du catalogue** : ses 16 Gio de fichiers dépassent ce qu'un onglet peut charger (voir section 7) ; le **12B** est disponible grâce à un port produit pour Oh my AI (section 7 bis) ; la **31B** n'a toujours aucun port navigateur / The **E2B/E4B** ("Efficient") variants fit comfortably in WebGPU. The **26B A4B** has a community ONNX port but **was removed from the catalog**: its 16 GiB of files exceed what a tab can load (see section 7); the **12B** is available through a port built for Oh my AI (section 7 bis); the **31B** still has no browser port at all.
...
### 7 bis. **Gemma 4 (12B) — WebGPU** 🧪 (Expérimental / Experimental)
- **Taille / Size**: ~8.2 GB (décodeur q4f16 en quatre tranches, embeddings fp16 / q4f16 decoder in four shards, fp16 embeddings)
- **Mémoire / Memory**: ~12 GB (poids, cache clé/valeur à 8 192 jetons, activations / weights, key/value cache at 8,192 tokens, activations). Pic mesuré dans Chrome : 12,9 Go dans l'onglet, 5,6 Go dans le processus GPU / Peak measured in Chrome: 12.9 GB in the tab, 5.6 GB in the GPU process.
- **Moteur / Engine**: **Transformers.js** (ONNX Runtime Web), *pas* WebLLM/MLC
- **Modèle / Model**: [`Dramatik999/gemma-4-12B-it-ONNX`](https://huggingface.co/Dramatik999/gemma-4-12B-it-ONNX), **port produit pour Oh my AI** à partir de `google/gemma-4-12B-it` / **port built for Oh my AI** from `google/gemma-4-12B-it`
- **Notes importantes / Important notes**:
  - **Pourquoi un port maison / Why a home-made port**: au 24 septembre 2026, aucun autre export du 12B ne se charge dans Transformers.js. Google ne publie que des safetensors et des GGUF ; `onnx-community` n'a que E2B et E4B ; `mlc-ai` n'a aucun Gemma 4 ; le seul export ONNX existant (`justinchuby/gemma-4-12b-onnx`) vise onnxruntime-genai / as of September 24, 2026, no other export of the 12B loads in Transformers.js. Google only publishes safetensors and GGUF; `onnx-community` only has E2B and E4B; `mlc-ai` has no Gemma 4; the only existing ONNX export targets onnxruntime-genai.
  - **Production** : `scripts/spike-gemma4-onnx/export_transformersjs.py`, avec `torch.onnx.export` en mode dynamo, sans opérateur spécifique. Le 12B se déclare `gemma4_unified`, inconnu de Transformers.js ; le `config.json` publié le présente comme `gemma4`, qui ne sert côté JavaScript qu'à choisir les sessions et les noms du cache / with `torch.onnx.export` in dynamo mode, no custom operator. The 12B declares `gemma4_unified`, unknown to Transformers.js; the published `config.json` presents it as `gemma4`, which on the JavaScript side only selects the sessions and cache names.
  - **Validation contre PyTorch / Validation against PyTorch**: décodeur fp16 via Transformers.js, 32 jetons gloutons sur 32 identiques ; q4f16, prompt de 3 679 jetons dont la réponse se trouve 3 000 jetons en arrière, 14 sur 14 identiques ; dans l'application sur WebGPU, même réponse juste en environ 26 s / fp16 decoder through Transformers.js, 32 of 32 greedy tokens identical; q4f16, 3,679-token prompt whose answer lies 3,000 tokens back, 14 of 14 identical; in the app on WebGPU, same correct answer in about 26 s.
  - ⚠️ **Texte seul** : l'encodeur vision n'est pas exporté, le bouton 🖼️ n'apparaît donc pas / ⚠️ **Text only**: the vision encoder is not exported, so the 🖼️ button does not appear.
  - **Contexte borné à 8 192 jetons** : les couches à fenêtre glissante gardent tout leur passé au lieu de le tronquer à 1 024, correct mais environ 340 Ko de cache par jeton / **Context capped at 8,192 tokens**: sliding-window layers keep their whole past instead of cropping it to 1,024, correct but about 340 KB of cache per token.
...
| Gemma 4 (12B) 🧪 | ~8.2 GB | ~12 GB | ⚡ | ⭐⭐⭐⭐⭐ | Meilleur Gemma 4 navigateur, texte seul, port maison |
